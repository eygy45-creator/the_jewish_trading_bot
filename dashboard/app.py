...
def main() -> None:
    ...
    st.subheader("Strict Prop Simulation — $5K Funded Account")
    strict_csv_path = PROJECT_ROOT / "data" / "live" / "strict_prop_simulation.csv"
    strict_json_path = PROJECT_ROOT / "data" / "live" / "strict_prop_summary.json"
    strict_df, strict_df_err = _read_csv_safe(strict_csv_path)
    strict_summary: dict[str, object] | None = None
    strict_summary_err: str | None = None
    if strict_json_path.is_file():
        try:
            payload = json.loads(strict_json_path.read_text(encoding="utf-8"))
            strict_summary = payload if isinstance(payload, dict) else None
            if strict_summary is None:
                strict_summary_err = "invalid_json"
        except (OSError, json.JSONDecodeError):
            strict_summary_err = "parse_error"
    else:
        strict_summary_err = "missing_file"
    ...
    missing_cols = _missing_required_columns(strict_df, required_cols) if strict_df_err is None and not strict_df.empty else required_cols
